...
object EnderecoParser:
  import Parser.*

  val logradouro: Parser[String] =
    regex("[A-Za-z ]+")

  val numero: Parser[Int] =
    digits

  val cidade: Parser[String] =
    regex("[A-Za-z ]+")

  val estado: Parser[String] =
    regex("[A-Z]{2}")

  val cep: Parser[String] =
    regex("[0-9]{5}-[0-9]{3}")

  val endereco: Parser[Endereco] =
    for
      log <- logradouro
      _ <- string(", ")
      num <- numero
      ...
      _ <- string(", ")
      c <- cep
    yield Endereco(log, num, cid, est, c)
...
@main def run(): Unit =
  ...
  enderecos.foreach { input =>
    EnderecoParser.endereco.parse(input) match
      case Some(ParseResult(endereco, remaining)) =>
        println(s"Parsed: $endereco")
