...
object DebugMacros:
  ...
  private def assertPositiveImpl(value: Expr[Double])(using Quotes): Expr[Double] =
    ...
    '{
      val v = $value
      if v <= 0 then
        throw new IllegalArgumentException(s"Valor deve ser positivo: ${$valueString} = $v")
      v
    }
...
case class Cliente(nome: String, cpf: String, saldo: Double)

case class Produto(nome: String, preco: Double, estoque: Int)

case class Endereco(rua: String, cidade: String, estado: String, cep: String)
