...
public class Main {
    public static void main(String[] args) {
        System.out.println("=== Sistema de Chatbot ===\n");
        ...
        System.out.println("Estrategia: Keyword Matching");
        ChatbotSystem keywordBot = new ChatbotSystem(new KeywordMatchingStrategy());
        for (String msg : messages) {
            ChatResponse response = keywordBot.sendMessage(msg);
            System.out.printf("  Usuario: %s%n", msg);
            System.out.printf("  Bot: %s (Confianca: %.2f)%n%n", response.text(), response.confidence());
        }

        System.out.println("\nEstrategia: Pattern Matching");
        ChatbotSystem patternBot = new ChatbotSystem(new PatternMatchingStrategy());
        String patternMsg = "Qual o preco do teclado mecanico?";
        ChatResponse patternResponse = patternBot.sendMessage(patternMsg);
        System.out.printf("  Usuario: %s%n", patternMsg);
        System.out.printf("  Bot: %s (Confianca: %.2f)%n", patternResponse.text(), patternResponse.confidence());

        System.out.println("\nEstrategia: Intent-Based");
        ChatbotSystem intentBot = new ChatbotSystem(new IntentBasedStrategy());
        String intentMsg = "Bom dia, gostaria de saber sobre pagamento e entrega";
        ChatResponse intentResponse = intentBot.sendMessage(intentMsg);
        System.out.printf("  Usuario: %s%n", intentMsg);
        System.out.printf("  Bot: %s (Confianca: %.2f)%n", intentResponse.text(), intentResponse.confidence());

        System.out.println("\nHistorico da Conversa (Intent-Based):");
        List<Message> history = intentBot.getHistory();
        System.out.printf("  Total de mensagens: %d%n", history.size());
    }
}
