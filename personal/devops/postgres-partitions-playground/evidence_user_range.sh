...
echo ""
echo "1.3 Show partition information (standard catalogs):"
docker exec postgres-partitions psql -U postgres -d partitions_db -c "
SELECT
...
WHERE parent.relname IN ('user_range_v1', 'user_range_v2')
...
docker exec -i postgres-partitions psql -U postgres -d partitions_db <<EOF
\\timing on
