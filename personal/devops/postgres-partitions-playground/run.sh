...
wait_for_postgres() {
    ...
    while [ $attempt -le $max_attempts ]; do
        if docker exec postgres-partitions psql -U postgres -d partitions_db -c "SELECT 1;" &> /dev/null; then
...
docker exec postgres-partitions psql -U postgres -d partitions_db -c "
...
echo ""
echo -e "${BLUE}Showing PostgreSQL logs. Press Ctrl+C to stop viewing logs and keep the container running in background.${NC}"
docker logs -f postgres-partitions
